...
#define SFTBLT_SETUP(T)															\
	...
	const int32 iter_w = dr_w + dest_rect->x;	\
	const int32 iter_h = dr_h + dest_rect->y;	\
	const uint32 src_pitchinpix = src_surface->pitchinpix;										\
	const uint32 dest_pitchinpix = dest_surface->pitchinpix;									\
	const int32 dest_pixels_fudge_x = dest_rect->x;									\
	const int32 dest_pixels_fudge_y = dest_rect->y;									\
	const T* src_pixels = src_surface->pixels + src_rect->x + (src_rect->y * src_pitchinpix);					\
	T* dest_pixels = dest_surface->pixels + dest_rect->x + (dest_rect->y * dest_pitchinpix);

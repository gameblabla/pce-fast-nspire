...
int mednafen(int argc, char *argv[]);
...
static int DoArgs(int argc, char *argv[], char **filename, char* romname)
{
	int ShowCLHelp = 0;
	...
	else
	{
	 ...
	 if(!ParseArguments(argc - 1, &romname, MDFNArgs, filename))
	  return(0);
...
void PumpWrap(void)
{
 ...
 if(Debugger_IsActive() || CheatIF_Active() || Help_IsActive())
 {
  if(!krepeat)
...
int mednafen(int argc, char *argv[], char* romname)
{
	//ThreadTest();
	...
	MDFNI_printf(_("Starting Mednafen...\n"));
	MDFN_indent(1);
	...
	if(!DoArgs(argc,argv, &needie, romname))
	{
	 MDFNI_Kill();
	 DeleteInternalArgs();
	 KillInputSettings();

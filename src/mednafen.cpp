...
MDFNGI *MDFNI_LoadCD(const char *force_module, const char *path)
{
 ...
 try
 {
        ...
        MDFN_printf(_("Using module: %s(%s)\n"), MDFNGameInfo->shortname, MDFNGameInfo->fullname);
	{
	 ...
	 {
	  std::string modoverride_settings_file_path = MDFN_GetBaseDirectory() + std::string(PSS) + std::string(MDFNGameInfo->shortname) + std::string(".cfg.tns");
...
int MDFNI_Initialize(const char *basedir, const std::vector<MDFNSetting> &DriverSettings)
{
	...
	try
	{
	 settings_file_path = std::string(basedir) + std::string(PSS) + std::string("mednafen-09x.cfg.tns");
